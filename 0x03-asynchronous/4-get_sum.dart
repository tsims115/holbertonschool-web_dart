...
Future<double> calculateTotal() async {
  ...
  var id = data.replaceAll('"', "").split(": ")[1].split(", ")[0];
  var orders = await fetchUserOrders(id);
  var orders1 = orders.replaceAll('"', "").replaceAll("[", "").replaceAll("]", "").split(",");
  return double.parse(await fetchProductPrice(orders1[0])) + double.parse(await fetchProductPrice(orders1[1]));
}
